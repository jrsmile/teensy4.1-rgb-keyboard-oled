...
// Gif Stuff
AnimatedGIF gif;
static uint8_t ucOLED[2048]; // holds current frame for 256x64 OLED
#define DISPLAY_WIDTH 256
#define DISPLAY_HEIGHT 64

// Function Declaration for Platform.io Compatibility
void scanKeys();
void scanEncoder();
void welcome();
void poweroff();
  ...
  while (1) {
    ...
    if (knobVal != 0) {
      if (knobVal >= 1) {
        ...
        u8g2.drawStr(0, 16, "quieter"); // write something to the internal memory
        ...
        u8g2.clearBuffer();         // clear the internal memory
        u8g2.setFont(u8g2_font_unifont_t_latin); // choose a suitable font
        u8g2.drawStr(0, 16, "louder"); // write something to the internal memory
        u8g2.setFont(u8g2_font_open_iconic_all_4x_t);
    ...
    if (knobSWVal != knobSWlast) {
      if (knobSWVal) {
        ...
        u8g2.clearBuffer();         // clear the internal memory
        u8g2.setFont(u8g2_font_unifont_t_latin); // choose a suitable font
        u8g2.drawStr(0, 16, "MUTE"); // write something to the internal memory
        u8g2.setFont(u8g2_font_open_iconic_all_4x_t);
        u8g2.drawStr(200, 32, "w"); // write something to the internal memory
        u8g2.setFont(u8g2_font_unifont_t_latin); // choose a suitable font
        u8g2.sendBuffer();          // transfer internal memory to the display
